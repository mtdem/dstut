...
#https://github.com/mlverse/torch/issues/972
x <- fitted %>% 
  predict(test_ds) %>% 
  torch_argmax(dim = 2) #%>%  # the predicted class is the one with higher 'logit'.
as_array(x$to(device="cpu")) %>% # we convert to an R object
  accuracy(truth)
```
...
x <- predict(fitted, xrnn[!istrain,, ])
kpred <- as.numeric(as_array(x$to(device="cpu")))
...
plot(fitted)
x <- predict(fitted, xrnn[!istrain, ,])
npred <- as.numeric(as_array(x$to(device="cpu")))
